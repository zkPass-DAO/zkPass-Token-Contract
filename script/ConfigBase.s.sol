...
contract ConfigBASE is Script {
    ...
    function SetLibraries() public {
        console2.log("SetLibraries for BASE...");

        address sendLib = vm.envAddress("BASE_SEND_LIB_ADDRESS"); // SendUln302 address
        address receiveLib = vm.envAddress("BASE_RECEIVE_LIB_ADDRESS"); // ReceiveUln302 address
        ...
        ILayerZeroEndpointV2(ENDPOINT).setReceiveLibrary(
            BASE_CONTRACT,
            BASE_CHAIN_EID, // Source chain EID -> BASE
            receiveLib,
            gracePeriod
        );
    }
    ...
    function SetSendConfig() public {
        console2.log("SetSendConfig for BASE...");
        ...
        address baseExecutor = vm.envAddress("BASE_EXECUTOR");
        ExecutorConfig memory baseExec = ExecutorConfig({
            maxMessageSize: 10000, // max bytes per cross-chain message
            executor: baseExecutor // address that pays destination execution fees on B
        });

        bytes memory encodedBaseUln = abi.encode(baseUln);
        bytes memory encodedBaseExec = abi.encode(baseExec);
    ...
    function SetReceiveConfigForETH() public {
    ...
    function SetPeerConfig() public {
        console2.log("SetPeerConfig for BASE...");
    ...
    function SetEnforcedOptions() public {
        console2.log("SetEnforcedOptions for BASE...");
        ...
        ZKPToken(BASE_CONTRACT).setEnforcedOptions(enforcedOptions);
    }

    function run() public {
        console2.log("Setting Config for BASE...");
